...
namespace Microsoft.Unity.VisualStudio.Editor {
	internal class VisualStudioAntigravityInstallation : VisualStudioInstallation {
		...
		public static IEnumerable<IVisualStudioInstallation> GetVisualStudioInstallations() {
			...
			// Search all drives for Program Files\Antigravity
			try {
				var drives = System.IO.DriveInfo.GetDrives();
				foreach (var drive in drives) {
					if (drive.IsReady && drive.DriveType == System.IO.DriveType.Fixed) {
						// Check D:\Program Files\Antigravity, E:\Program Files\Antigravity, etc.
						candidates.Add(IOPath.Combine(drive.Name, "Program Files", "Antigravity", "Antigravity.exe"));
						candidates.Add(IOPath.Combine(drive.Name, "Program Files (x86)", "Antigravity", "Antigravity.exe"));
					}
				}
			} catch {
